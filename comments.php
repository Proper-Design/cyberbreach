...
<div id="respond">
	...
	<h2><?php comment_form_title( __('Leave a Reply','properbear'), __('Leave a Reply to %s','properbear') ); ?></h2>
	...
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
		...
		<?php else : ?>
			...
			<div>
				...
				<label for="author"><?php _e('Name','properbear'); ?> <?php if ($req) echo "(required)"; ?></label>
			</div>

			<div>
				<input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
				<label for="email"><?php _e('Mail (will not be published)','properbear'); ?> <?php if ($req) echo "(required)"; ?></label>
			</div>
